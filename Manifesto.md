...
- **L8: Double value, every loop leaves a trace.** Every useful interaction moves the work forward **and** produces a reusable trace. The bridge between Execution (4) and Memory (6). → [double-valeur](concept/double-valeur.md) *(borrowed from JP Noto's LIVING REFERENCE — the source record is [research/06](concept/research/06-living-reference.md))*.

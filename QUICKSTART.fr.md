...
2. **Copier le MOS par défaut.** Récupérer le cadre, puis copier le système prêt à l'emploi dans un conteneur à lui :
   ...
   Tout le reste est dans la copie : fichiers d'identité, table de routage, 7 skills de base (dont `mos-map`, la carte visuelle de ton système), hooks de sécurité, knowledge-base — plus `BOOTSTRAP.md`, le rituel de premier démarrage.

...
## 19. Seeing the MOS: the visual map (`mos-map`)

A MOS is files in a terminal; the `mos-map` skill gives it a face. One command generates **one self-contained HTML file** — no server, no network, no dependency beyond python3's standard library — that opens in any recent browser, offline: the hexagon with its attachments (production at the north vertex, the knowledge base at the south, connectors on the diagonals), a file explorer, the local knowledge graph with full-text search, the journal as a timeline. The map is a **dated snapshot**: refreshing it means regenerating it. Nothing on it is drawn by hand; everything is read from the real files — a map that could drift from its system would be worse than no map.

Connectors reach the map through **`.claude/mos-map.json`** (schema 2): one *attachment* per connector — vertex (0 = north, then clockwise to 5), nature (`kb | production | mos | git | externe`), display name, one-line summary, outbound links, and `visible: false` to declare without displaying. The map edits its own declaration (drag a card onto a vertex, add from a corner's ＋, save back to the file) — but **the file remains the source of truth**, and `connect-adapter` keeps it aligned with the CLAUDE.md connector map: two synchronized homes — the prose is authoritative, the declaration is mechanical. A **confidential** adapter never enters `mos-map.json`: the file is versioned and shared, and §12's zero-knowledge applies to maps too.

Two disclosure profiles govern what the generated file embeds: `interne` (everything — for the operator's own eyes; the file then shares like a sensitive document) and `public` (structure, counts and titles only — no excerpts, no descriptions, no inbox, no machine paths). A map meant for marketing uses a synthetic dataset: even titles are facts.

# AGENTS.md, <nom du projet> (cœur Manence OS)

Ce fichier est l'identité du système, et il est neutre vis-à-vis de l'agent : quel que soit l'agent de code que tu es (Claude Code, OpenAI Codex, Grok Build), c'est ce que tu lis en premier.
...
## Skills
Base (fournis par le cadre) : `open-work` (ouvrir un chantier), `close-work` (publier un chantier), `weekly-review` (revue hebdo : lint, inbox, chantiers), `kb-ingest` (intégrer une source au savoir), `kb-lint` (audit de la KB), `connect-adapter` (brancher un adaptateur), `outward-watch` (regarder dehors : le substrat et le métier).
...
## Sécurité, dur vs mou
- **Mou (ce fichier)** : ce fichier ne fait que *suggérer*. L'agent peut s'en écarter.
- **Dur (ce qui contraint vraiment)** : un hook `PreToolUse` ou une permission refusée, déclarés dans la configuration propre à l'agent. Pour *interdire* (ex. `rm -rf`, push direct), c'est là, pas ici. Les garde-fous eux-mêmes sont livrés une seule fois, sous `.claude/` : `.claude/hooks/guard.sh` (bloque) et `.claude/hooks/lint.sh` (rapporte). Chaque harnais les câble à sa façon — Claude Code : `.claude/settings.json` + ces hooks ; OpenAI Codex : `.codex/config.toml` ; Grok Build : sa compatibilité Claude native, rien à câbler.

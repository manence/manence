---
name: close-work
description: Clôt un chantier de la production en le déplaçant de <domaine>/in-progress/ vers <domaine>/done/YYYYMMDD-<slug>/, avec distillation obligatoire (faits durables vers la KB, bilan au log) : les artefacts de production sont jetables, ce skill est le seul garant de durabilité. À utiliser quand un livrable est terminé et validé, quand un chantier est abandonné, ou pour régulariser un chantier mal né (fiche rétroactive).
---
...
2. **Clore** : déplacer `<domaine>/in-progress/<slug>/` vers `<domaine>/done/YYYYMMDD-<slug>/` (la date du jour ; le préfixe fait le tri chronologique). La racine de production est `$<PROJET>_PRODUCTION_ROOT` (défaut `../production/`), résolue en absolu, jamais devinée (Spec §18).

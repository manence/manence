...
2. **Copy the default MOS.** Get the framework, then copy the ready-made system into a container of its own:
   ...
   Everything else is in the copy: identity files, the routing table, 8 base skills (including mos-map, the visual map of your system, and outward-watch, its eye on the outside), safety hooks, knowledge base — plus `BOOTSTRAP.md`, the one-time startup ritual.
